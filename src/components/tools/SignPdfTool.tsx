    ...
    processAndDownload: async () => {
      ...
      try {
        ...
        const finalOverlays = await Promise.all(overlays.map(async (overlay) => {
          if (overlay.type === 'text' && overlay.fontFamily) {
            const fontObj = fonts.find(f => f.name === overlay.fontFamily);
            if (fontObj) {
              const res = await fetch(fontObj.url);
              const fontBytes = await res.arrayBuffer();
              return { ...overlay, fontBytes };
            }
          }
          return overlay;
        }));
                ...
                >
                  <div className="w-full h-full relative flex items-center justify-center">
                    {overlay.type === 'image' ? (
                      ...
                      <svg viewBox="0 0 400 100" className="w-full h-full drop-shadow-sm pointer-events-none" preserveAspectRatio="xMidYMid meet">
                         <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle" style={{ fontSize: '48px', fontFamily: overlay.fontFamily, fill: overlay.color }}>
                           {overlay.content}
                         </text>
                      </svg>
                    )}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#252525] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-black/10 dark:border-white/10 flex flex-col">
            ...
            <div className="p-6 flex-1 flex flex-col">
              {signatureMode === 'draw' && (
                <>
                  <div className="flex-1 bg-[#F9F9F9] dark:bg-[#1E1E1E] rounded-xl border border-black/10 dark:border-white/10 overflow-hidden relative min-h-[200px]">
                    <canvas
                      ref={sigCanvasRef}
                      width={450}
                      height={200}
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-[#F9F9F9] dark:bg-[#1C1C1E] border border-black/10 dark:border-white/10 rounded-xl text-lg text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                    />
                  </div>
                  <div>
                    <label className="block text-[12px] font-medium text-black/60 dark:text-white/60 mb-2">Select Style</label>
                    <div className="grid grid-cols-1 gap-2">
                      {fonts.map(f => (
                        <button
                          key={f.name}
                          onClick={() => setSelectedFont(f.name)}
                          className={`flex items-center justify-between p-4 rounded-xl border transition-all ${selectedFont === f.name ? 'border-blue-500 bg-blue-50 dark:bg-blue-500/10' : 'border-black/10 dark:border-white/10 bg-[#F9F9F9] dark:bg-[#1C1C1E] hover:border-black/20 dark:hover:border-white/20'}`}
                        >
              ...
              {signatureMode === 'upload' && (
                <div className="flex-1 flex flex-col items-center justify-center min-h-[200px] border-2 border-dashed border-black/10 dark:border-white/10 rounded-xl bg-[#F9F9F9] dark:bg-[#1E1E1E] hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer" onClick={() => document.getElementById('sig-upload')?.click()}>
